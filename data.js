...
const START_DEX_ID = 1;
const END_DEX_ID = 565; // As per your request
...
async function scrapePokemonPage(dexId) {
  ...
  try {
    ...
    if (article.length === 0) {
      console.warn(
        `[WARNING] Could not find the target HTML block for Dex ID ${dexId}. Skipping.`
      );
    ...
    // 2. Pokedex Entry
    const pokedexEntry = article.find('figure.pokedex-entry blockquote');
    pokemon.pokedexEntry = pokedexEntry.text().trim();
    ...
    // 6. Abilities (NOT present in the provided HTML, so omitted from output)
...
async function main() {
  console.log(
    `Starting scraping process for Dex IDs ${START_DEX_ID} to ${END_DEX_ID}...`
  );
